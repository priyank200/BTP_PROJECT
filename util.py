...
def map_label(label, classes):
    """
    This function is used to map labels to a new set of indices. It takes the original labels (label) and a set of classes (classes). It assigns a new index to each label based on its position in the provided set of classes.
    """
...
class DATA_LOADER(object):
    ...
    def read_matdataset(self, opt):
        ...
        label = matcontent['labels'].astype(int).squeeze() # minus one htya hai mne
        ...
        self.allclasses = np.arange(0, self.ntrain_class+self.ntest_class).astype(np.int) # added one here
        
        self.train_mapped_label = map_label(self.train_label, self.seenclasses) 

    
    def next_batch(self, batch_size):
        """This method is used to get the next batch of data. It randomly selects indices, retrieves features, labels, and attributes for the batch, and returns them"""
        ...
        # print(all(all(element == batch_att[0][0] for element in inner_lst) for inner_lst in batch_att))
